...
exports.run = (client, message, args) => {
    if (channels.includes(message.channel.id)) {
       message.channel.send(new Discord.RichEmbed().setColor("0x36393e").setAuthor('Hata').setDescription("- Bir kanalda birden fazla **Resim** çizemessiniz!"));
        return;
    }
    var size;
    if (+args[0] >= 5 && +args[0] <= 50) {
        ...
        message.channel.send(new Discord.RichEmbed().setColor("0x36393e").setAuthor('Hata').setDescription("- Lütfen resim boyutunu **5** ile **50** arasında seçmelisiniz!"));
        return;
    }
...
class Drawing {
    constructor(msg, size, fg, bg) {
        ...
        msg.edit("--------------------------------------------------------------------------------\n⬅ - **Sola** Git | ➡ - **Sağa** Git | ⬆ - **Yukarıya** Çık | ⬇ - **Aşşağıya** İn \n--------------------------------------------------------------------------------\n ✅ - Çizimi **Durdur** | 🖋 - Kalem **Açık** | 🔏 - Kalem **Kapalı**\n--------------------------------------------------------------------------------");
        this.reactArrows(0);
    ...
    handleReaction(reaction) {
        ...
        reaction.remove(reaction.users.last()).catch(e => {
            if (e.code === 50013) reaction.message.channel.send(new Discord.RichEmbed().setColor(ayarlar.renk).setAuthor('Hata').setDescription("- Düzgün çalışması için **Mesajları Yönet** iznine ihtiyacım var!"));
        });
        this.drawCanvas();
    }
    ...
    async sendCanvas() {
        ...
        this.msg.channel.send(``/*Resim: **${this.size}** pixel*/, { 
            files: [
                this.c.toBuffer()
            ]
        }).then(msg => {
...
enabled: true,
guildOnly: false,
aliases: ['resimçiz', 'art', 'resim', 'çiz'],
kategori: `eğlence`,  
permLevel: `Gerekli yetki bulunmuyor`
};

exports.help = {
name: "resim-çiz",
description: "Kendiniz emojiler ile resim çizersiniz.",
usage: "resim-çiz"
};